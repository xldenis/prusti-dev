...
impl<'mir, 'tcx: 'mir> DefinitelyAccessibleAnalysis<'mir, 'tcx> {
    ...
    pub fn run_analysis(
        ...
        let location_table = &self.body_with_facts.location_table.as_ref().expect("expected location table to be present");
        let borrowck_out_facts = self.body_with_facts.output_facts.as_ref().expect("expected polonius output to be present");
